...
export default function Newlist() {  
    ...
    const newProduct = product.filter((item)=>(
            item.productCatalog === "new"
    ))
    const [ newList, setNewList ] = useState(newProduct.slice(0,5))
            ...
            <div className="new__container bd-grid">
                <div className="new__mens">
                    ...
                    <span className="new__price">From ${newList[0].productPrice}</span>
                     <Link to={`/product/${newList[0].productId}`}><a href="#" className="button-light">View Collection<BiRightArrowAlt/></a></Link>
                </div>
                <div className="new__sneaker">
                    {newList.filter((item,index)=>index>0).map((newItem) => {
                        return <New key={newItem.productId} 
